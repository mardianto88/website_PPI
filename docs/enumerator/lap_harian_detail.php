...
$pdf->Ln(0.1);
$pdf->SetX(4);
$pdf->MultiCell(29.5,0.5,'LAPORAN DATA HARIAN JUMLAH DAN JENIS IKAN',0,'C');    
...
$tgl   = $_GET['tgl']; 
$query=mysqli_query($koneksi, "SELECT hasil.id_transaksi, hasil.tgl, km.nama_km,  ikan.jenis_ikan, hasil.kg, hasil.harga, hasil.jumlah_harga FROM km inner join hasil on km.id_km=hasil.id_km inner join ikan on ikan.id_ikan=hasil.id_ikan where hasil.tgl='$tgl'");
...
$jumlah_harga = mysqli_query($koneksi, "select format(sum(jumlah_harga),0) from  hasil where hasil.tgl='$tgl'");
